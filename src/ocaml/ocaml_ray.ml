...
let mag (x : float) (y : float) (z : float) : float = sqrt ((sq x)+.(sq y)+.(sq z))

let unit_vector (x : float) (y : float) (z : float) : point =
  let d = mag x y z in
  {x = (x/.d); y = (y/.d); z = (z/.d)}
...
let world : sphere array = Array.make 33 {color=0.0;radius=0.0;center={x=0.0;y=0.0;z=0.0}}

let eye : point = {x=0.0;y=0.0;z=200.0}

let sphere_normal (s : sphere) (pt : point) : point =
  let c = s.center in
  unit_vector (c.x-.pt.x) (c.y-.pt.y) (c.z-.pt.z)        
        
let lambert (s : sphere) (i : point) (ray : point) : float =
  let n = sphere_normal s i in
  max 0.0 ((ray.x*.n.x)+.(ray.y*.n.y)+.(ray.z*.n.z))
  
let rec loop (pt : point) (ray : point) (index : int)
          (lst_len : int) (lst : sphere array)
          (surface : sphere option) (hit : point option) (dist : float)
        : (sphere option * point option) =
...
let sendray (pt : point) (ray : point) : float =
...
let color_at (x : float) (y : float) : int =
