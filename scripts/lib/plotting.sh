...
# runtime.sh should be sourced before calling any of these functions
...
function plot()
{
    ...
    plot_benchmark "quicksort" $c1 $c2 "$c1t" "$c2t" "$ct" $dyn_config
    plot_benchmark "fft" $c1 $c2 "$c1t" "$c2t" "$ct" $dyn_config
    plot_benchmark "blackscholes" $c1 $c2 "$c1t" "$c2t" "$ct" $dyn_config
    plot_benchmark "matmult" $c1 $c2 "$c1t" "$c2t" "$ct" $dyn_config
    plot_benchmark "n_body" $c1 $c2 "$c1t" "$c2t" "$ct" $dyn_config
    plot_benchmark "tak" $c1 $c2 "$c1t" "$c2t" "$ct" $dyn_config
    plot_benchmark "ray" $c1 $c2 "$c1t" "$c2t" "$ct" $dyn_config
    plot_benchmark "array" $c1 $c2 "$c1t" "$c2t" "$ct" $dyn_config
}
...
function plot_benchmark()
{
    ...
    local config1_cumulative_log="${DATA_DIR}/${name}${c1}.csv1"
    local config2_cumulative_log="${DATA_DIR}/${name}${c2}.csv1"
