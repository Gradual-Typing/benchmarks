...
get_chezscheme_runtime()
{
    ...
    if [ -f $cache_file ]; then
        ...
        echo "(compile-program \"${benchmark_path}.ss\")" | chez-scheme -q
...
get_grift_runtime()
{
    ...
    if [ -f $cache_file ]; then
        ...
        grift-bench --config $config_index "${benchmark_path}.grift"
...
geometric_mean()
{
    ...
    local indx="$1";    shift
    RETURN=$(cat "$logfile" | sed 1d | cut -d, -f$indx | "${LIB_DIR}/geo")
}
